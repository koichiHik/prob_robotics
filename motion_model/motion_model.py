...
class MotionErrorModel2D():
  ...
  def sample_motion(self, *, cur_odom, last_odom, last_particle_poses):
    ...
    nor_cur_odom[2] = self.__normalize_angle_between_0_to_2pi(nor_cur_odom[2])
    nor_last_odom[2] = self.__normalize_angle_between_0_to_2pi(nor_last_odom[2])
    ...
    odom_diff_mat = self.__generate_true_odom_diff_array(rot1, trans, rot2, nor_last_odom, particle_num)
  ...
  def __calc_sigma(self, rot1, trans, rot2):
    ...
    sigma_rot1 = self.conf._std_rot_per_rot * rot1**2 + self.conf._std_rot_per_trans * trans**2
    sigma_trans = self.conf._std_trans_per_trans * trans**2 + self.conf._std_trans_per_rot * rot2**2
    sigma_rot2 = self.conf._std_rot_per_rot * rot2**2 + self.conf._std_rot_per_trans * trans**2
  ...
  def __decompose_odom_diff(self, cur_odom, last_odom):
    ...
    rot1 = self.__normalize_angle_between_0_to_2pi(rot1)
    trans = math.sqrt(u[0]**2 + u[1]**2)
    u[2] = self.__normalize_angle_between_0_to_2pi(u[2])
    rot2 = u[2] - rot1
    rot2 = self.__normalize_angle_between_0_to_2pi(rot2)

    return rot1, trans, rot2

  def __normalize_angle_between_0_to_2pi(self, val1):
    value = val1
    if (2 * math.pi < val1):
  ...
  def __generate_true_odom_diff_array(self, rot1, trans, rot2, last_odom, particle_num):
    ...
    t_rot1_arr = rot1_arr - np.random.normal(0, sig_r1, particle_num)
    t_trans_arr = trans_arr - np.random.normal(0, sig_t, particle_num)
    t_rot2_arr = rot2_arr - np.random.normal(0, sig_r2, particle_num)

    d_x_arr = np.array(t_trans_arr) * np.array(np.cos(last_odom[2] + t_rot1_arr))
    d_y_arr = np.array(t_trans_arr) * np.array(np.sin(last_odom[2] + t_rot1_arr))
